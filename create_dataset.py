...
def is_test_problem(example):
        """Check if this problem should be in the test set based on manual identification"""
...
def create_aime_dataset_from_existing():
    ...
    def standardize_to_final_schema(example, is_2025_data=False):
        ...
        if 'Problem Number' in example and example['Problem Number'] is not None:
            ...
            try:
                result['Problem Number'] = int(example['id']) + 1
            except (ValueError, TypeError):
                result['Problem Number'] = 0
        else:
            result['Problem Number'] = 0
    ...
    filtered_test_dataset = combined_test_dataset.filter(is_test_problem)
...
def main():
    ...
    HF_TOKEN = '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' # Set your Hugging Face token as environment variable
